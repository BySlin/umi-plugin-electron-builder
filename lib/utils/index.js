...
var { execa, chalk } = import_umi.utils;
...
function isNpm() {
  const packageLockJsonPath = import_path.default.join(process.cwd(), "package-lock.json");
  return fse.pathExistsSync(packageLockJsonPath);
}
function isYarn() {
  const yarnLockPath = import_path.default.join(process.cwd(), "yarn.lock");
...
function getRootPkg() {
  const pkg = fse.readJSONSync(import_path.default.join(process.cwd(), "package.json"));
