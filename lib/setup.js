...
var setup_default = (api) => {
  (0, import_utils.setNpmClient)(api.userConfig.npmClient);
  const nodeModulesPath = (0, import_utils.getNodeModulesPath)();
  let rootPkg = (0, import_utils.getRootPkg)();
  ...
  if (installDependencies.length > 0) {
    (0, import_utils.installRely)(installDependencies.join(" "));
  }
  rootPkg = (0, import_utils.getRootPkg)();
  const electronPackageJson = fse.readJSONSync(import_path.default.join(nodeModulesPath, "electron", "package.json"));
  if (electronPackageJson.dependencies["@types/node"] !== rootPkg.devDependencies["@types/node"]) {
    ...
    (0, import_utils.installRely)(`@types/node@${electronTypesNodeVersion}`);
  }
  rootPkg = (0, import_utils.getRootPkg)();
  ...
  if (isUpdateRootPkg) {
    ...
    fse.writeFileSync(import_path.default.join(process.cwd(), "package.json"), JSON.stringify(rootPkg, null, 2));
  }
};
