...
var buildPreload = (api) => {
  ...
  if (fse.pathExistsSync((0, import_utils.getPreloadSrc)(api))) {
    const tasks = [];
    if (buildType === "webpack") {
...
var runDev = async (api) => {
  ...
  const electronPath = require(import_path.default.join((0, import_utils.getNodeModulesPath)(), "electron"));
  let spawnProcess = null;
  const runMain = (0, import_utils.debounce)(() => {
    ...
    spawnProcess = (0, import_child_process.spawn)(String(electronPath), [
      `--inspect=${debugPort}`,
      import_path.default.join((0, import_utils.getDevBuildDir)(api), "main.js")
    ]);
    spawnProcess.stdout.on("data", (data) => {
      const log = (0, import_utils.filterText)(data.toString());
      if (log) {
  ...
  const buildMainDebounced = (0, import_utils.debounce)(() => buildMain(api), TIMEOUT);
  const buildPreloadDebounced = (0, import_utils.debounce)(() => buildPreload(api), TIMEOUT);
  const runPreload = (0, import_utils.debounce)(() => {
  ...
  const watcher = chokidar.watch(
    [
      ...
      `${(0, import_utils.getDevBuildDir)(api)}/**`
    ],
    { ignoreInitial: true }
  );
  watcher.on("unlink", (path2) => {
    if (spawnProcess !== null && path2.includes((0, import_utils.getDevBuildDir)(api))) {
      spawnProcess.kill("SIGINT");
      spawnProcess = null;
    }
  }).on("add", (path2) => {
    if (path2.includes((0, import_utils.getDevBuildDir)(api))) {
      return runMain();
    }
    ...
    if (path2.includes((0, import_utils.getMainSrc)(api))) {
      return buildMainDebounced();
    }
    if (path2.includes("main.js")) {
      return runMain();
    }
    if (path2.includes((0, import_utils.getPreloadSrc)(api))) {
